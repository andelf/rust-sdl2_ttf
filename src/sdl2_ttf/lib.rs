...
impl Font {
    ...
    pub fn from_file(filename: &Path, ptsize: i32) -> SdlResult<Font> {
        ...
        unsafe {
            ...
            let raw = ffi::TTF_OpenFont(cstring.as_ptr() as *const _, ptsize as c_int);
            if raw.is_null() {
                Err(get_error())
            } else {
    ...
    pub fn from_file_index(filename: &Path, ptsize: i32, index: i32) -> SdlResult<Font> {
        ...
        unsafe {
            ...
            let raw = ffi::TTF_OpenFontIndex(cstring.as_ptr() as *const _, ptsize as c_int, index as c_long);
            if raw.is_null() {
                Err(get_error())
            } else {
                Ok(Font { raw: raw, owned: true })
    ...
    pub fn face_family_name(&self) -> Option<String> {
        ...
        unsafe {
            ...
            if cname.is_null() {
                ...
                Some(String::from_utf8_lossy(CStr::from_ptr(cname as *const _).to_bytes()).to_string())
            }
        }
    }
    ...
    pub fn size<'a, T>(&self, text: T) -> SdlResult<(u32, u32)> where T: Into<Text<'a>> {
        ...
        let ret = unsafe {
            ffi::TTF_SizeText(self.raw, ctext.as_ptr() as *const _, &w, &h)
        };
        if ret != 0 {
            Err(get_error())
        } else {
        ...
        unsafe {
            let raw = match text.into() {
                ...
                Text::Latin1(bytes) => {
                    ...
                    match mode {
                        RenderMode::Solid { foreground } => {
                            ffi::TTF_RenderText_Solid(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground))
                        },
                        RenderMode::Shaded { foreground, background } => {
                            ffi::TTF_RenderText_Shaded(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground), color_to_c_color(background))
                        },
                        RenderMode::Blended { foreground } => {
                            ffi::TTF_RenderText_Blended(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground))
                        },
                        RenderMode::BlendedWrapped { foreground, wrap_length } => {
                            ffi::TTF_RenderText_Blended_Wrapped(self.raw, source.as_ptr() as *const _,
                ...
                Text::Utf8(string) => {
                    ...
                    match mode {
                        RenderMode::Solid { foreground } => {
                            ffi::TTF_RenderUTF8_Solid(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground))
                        },
                        RenderMode::Shaded { foreground, background } => {
                            ffi::TTF_RenderUTF8_Shaded(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground), color_to_c_color(background))
                        },
                        RenderMode::Blended { foreground } => {
                            ffi::TTF_RenderUTF8_Blended(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground))
                        },
                        RenderMode::BlendedWrapped { foreground, wrap_length } => {
                            ffi::TTF_RenderUTF8_Blended_Wrapped(self.raw, source.as_ptr() as *const _,
                                color_to_c_color(foreground), wrap_length)
                        }
                    }
                },
